...
common_options = [
    ...
    click.option('--link-in-comment', is_flag=True, callback=set_boolean_env_from_flag,
                 help='Post POST_LINK as a comment/reply on the published post instead of attaching it to the post'),
    click.option('--pin-link-comment', is_flag=True, callback=set_boolean_env_from_flag,
                 help='Pin the link comment (platform support varies; logs warning if unsupported)'),
    ...
    click.option('--log-level', callback=set_env_from_option, 
                 help='Logging level (DEBUG, INFO, WARNING, ERROR). GitHub Actions debug mode defaults to DEBUG when LOG_LEVEL is not set.'),
    click.option('--retry', callback=set_env_from_option,
                 help='Overall retry policy for network requests: <retries>*immediately, <retries>*delay(<seconds>), or <retries>*exp(<seconds>).'),
...
@click.option('--dailymotion-playlist-id', callback=set_env_from_option, 
              help='Dailymotion Playlist ID to add the video to')
